...
echo "bitcoinz-for-macos commit version : $git_rev_string"
echo "bitcoinz-for-macos tag version    : $git_tag_string"
echo "creating 'bitcoinz-macos-$release_version' release"

# prepare environment
source environment;
export BSPJOB=16;
export BSPINSTALL=${BSPROOT}/bitcoinz-macos-$release_version;

# build tools
make
echo $git_rev_string  > ${BSPINSTALL}/usr/local/bin/version.txt
echo $git_tag_string >> ${BSPINSTALL}/usr/local/bin/version.txt
echo "Bitcoinz for macOS ($release_version) is installed to ${BSPINSTALL}"

# create tarball and checksum
echo "creating Bitcoinz tarball 'bitcoinz-macos-$release_version.tar.bz2' and checksum"
tar -cjvf bitcoinz-macos-$release_version.tar.bz2 bitcoinz-macos-$release_version
shasum -a 256 bitcoinz-macos-$release_version.tar.bz2 > bitcoinz-macos-$release_version.tar.bz2.hash
shasum -a 256 -c bitcoinz-macos-$release_version.tar.bz2.hash
